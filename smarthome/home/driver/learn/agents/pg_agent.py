...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        advantages = self.estimate_advantage(observations, rewards_list, q, terminals)
        train_log = self.actor.update(observations, actions, advantages, q)
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            values = q_values.mean() + values_unnormalized * q_values.std()
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i]:
                        ...
                        _d = rews[i] + self.gamma * values[i+1] - values[i]
                        advantages[i] = advantages[i+1] * self.gamma * self.gae_lambda + _d
    ...
    def _discounted_return(self, rewards):
        ...
        list_of_discounted_returns = np.sum(np.power(self.gamma, range(len(rewards))) * rewards) * len(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for i in range(len(rewards)):
            reward_to_go = np.sum(rewards[i:] * np.power(self.gamma, np.arange(len(rewards)) - i)[i:])
            list_of_discounted_cumsums[i] = reward_to_go
